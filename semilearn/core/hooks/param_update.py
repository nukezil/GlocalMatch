...
class ParamUpdateHook(Hook):
    ...
    def after_train_step(self, algorithm):
        ...
        if algorithm.use_amp:
            ...
            if algorithm.clip_grad > 0:
                torch.nn.utils.clip_grad_norm_(algorithm.model.parameters(), algorithm.clip_grad)
            if type(algorithm.optimizer) is dict:
                for name in algorithm.optimizer.keys():
                    algorithm.optimizer[name].step()
            else:
                algorithm.optimizer.step()

        if algorithm.scheduler is not None:
            if type(algorithm.scheduler) is dict:
                for name in algorithm.scheduler.keys():
                    algorithm.scheduler[name].step()
            else:
                algorithm.scheduler.step()

        if type(algorithm.model) is dict:
            for name in algorithm.model.keys():
                algorithm.model[name].zero_grad()
        else:
